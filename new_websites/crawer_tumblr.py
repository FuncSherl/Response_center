...
'''
需要将site.txt放到与本文件同一目录下，其中txt中格式应为 :  abc,bdg,sdf,asdf   他们分别对应abc.tumblr.com   bdg.tumblr.com  ...
'''
...
class CrawlerScheduler(object):
    ...
    def _download_media(self, site, medium_type, start):
        ...
        target_folder = os.path.join(outdir, site)
        if not os.path.isdir(target_folder):
            os.mkdir(target_folder)

        base_url = "https://{0}.tumblr.com/api/read?type={1}&num={2}&start={3}"
        #https://queen570819.tumblr.com/api/read?type=photo&num=50&start=0
        start = START
        while True:
...
if __name__ == "__main__":
    if not op.isdir(outdir): os.makedirs(outdir)
